...
const Testimonial = () => {
  ...
  return (
    <>
    <section className='relative w-full h-[50rem] overflow-hidden'>
      <div className='flex justify-center text-yellow-500 top-0 text-[3rem] font-bold mt-20'>What Industry Leaders Say About HI-AIM</div>
    
      <div className='absolute inset-0'></div>

      <div className='relative z-10 mx-auto max-w-7xl px-4 mt-24'> 
            ...
            {pages.map((page, pageIndex) => (
              <div key={pageIndex} className=' flex-shrink-0 w-full flex justify-center gap-6 px-4 py-10' style={{ width: `${100 / pages.length}%` }}>
                {page.map((item, itemIndex) => (
                  <article
                    key={itemIndex}
                    className='relative w-full max-w-sm h-96 rounded-md border border-white/20 bg-white/10 p-8 shadow-xl backdrop-blur-xl flex flex-col justify-between bg-gradient-to-r from-gray-700 via-gray-900 to-black'
                  >
